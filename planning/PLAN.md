...
- **Frontend**: Next.js with TypeScript, built as a static export (`output: 'export'`), served by NestJS as static files
...
├── backend/                  # NestJS TypeScript project
...
├── Dockerfile                # Multi-stage build (Next.js → NestJS)
...
- **`frontend/`** is a self-contained Next.js project. It knows nothing about the backend. It talks to the backend via `/api/*` endpoints and `/api/stream/*` SSE endpoints. Internal structure is up to the Frontend Engineer agent.
- **`backend/`** is a self-contained NestJS project with its own `package.json` and `tsconfig.json`. It owns all server logic including database initialization, schema, seed data, API routes, SSE streaming, market data, and LLM integration. Internal structure is up to the Backend/Market Data agents.
...
Stage 2: Node 20 slim
  - Copy frontend/ build output into static/ directory
  - Copy backend/
  - npm install --production (NestJS backend)
  - npm run build (compile TypeScript)
  - Expose port 8000
  - CMD: node dist/main.js
```

NestJS serves the static frontend files and all API routes on port 8000.
